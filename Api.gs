...
function doGet(e) {
  try {
    ...
    switch (action) {
      ...
      case "rotacion":          return _handleRotacion(params);
      case "calendario":        return _handleCalendario(params);
...
function _handleRotacion(params) {
  const rodeoId = (params.rodeo_id || "").trim();
  const dias    = params.dias ? parseInt(params.dias) : 90;
  if (!rodeoId) return _respuestaError("Falta rodeo_id.", 400);
  const resultado = calcularRotacion(rodeoId, dias);
  if (!resultado.ok) return _respuestaError(resultado.error, 422);
  return _respuestaOk(resultado);
}

function _handleCalendario(params) {
  const dias = params.dias ? parseInt(params.dias) : 90;
  const resultado = calcularCalendarioEstablecimiento(dias);
  if (!resultado.ok) return _respuestaError(resultado.error, 422);
  return _respuestaOk(resultado);
}
